...
import * as PieceQueue from "../modules/piece-queue.js"
...
export var update = function (game, action) {
    ...
    switch (action) {
        ...
        case "TOGGLE_PAUSE": {
            if (game.state === "PLAYING") return __assign(__assign({}, game), { state: "PAUSED" })
            if (game.state === "PAUSED") return __assign(__assign({}, game), { state: "PLAYING" })
            return game
        }
        case "HARD_DROP": {
        ...
        case "HOLD": {
            ...
            if (game.heldPiece && !isEmptyPosition(game.matrix, __assign(__assign({}, game.piece), { piece: game.heldPiece.piece }))) {
                return game
            }
            var next = PieceQueue.getNext(game.queue)
            var newPiece =
                (_b = (_a = game.heldPiece) === null || _a === void 0 ? void 0 : _a.piece) !== null && _b !== void 0 ? _b : next.piece
